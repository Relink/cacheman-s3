 ...
 * @param {number|object} options { [ttl] - in seconds | { ttl, fromDate - in miliseconds }
 * @param {function} fn - The callback that will receive an error or data about the result
 */
var set = function (key, data, options, fn) {
...
var get = function (key, options, fn) {
  ...
  ctx.client.headObject({
    ...
    if (data.Metadata && data.Metadata.FromDate) {
      if (data.Metadata.FromDate < options.fromDate) {
        return fn(null, null);
      }
    }
